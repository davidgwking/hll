...
var ESAMPLE_TOO_LARGE = 'Bit sample size must be at most 12.';
var ESAMPLE_TOO_SMALL = 'Bit sample size must be at least 4.';
var EMUST_BE_INTEGER  = 'Bit sample size must be an integer.';
var EMUST_BE_STRING_F = 'Estimator input must be string, but found %s.';

function hll(b) {
  if (b % 1 !== 0) throw new RangeError(EMUST_BE_INTEGER);
  if (b < 4) throw new RangeError(ESAMPLE_TOO_LARGE);
  if (b > 12) throw new RangeError(ESAMPLE_TOO_SMALL);
  ...
  function insert(value) {
    if (typeof value !== 'string')
      throw new TypeError(fmt(EMUST_BE_STRING_F, typeof value));
